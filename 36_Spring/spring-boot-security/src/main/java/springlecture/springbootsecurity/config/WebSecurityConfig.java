...
public class WebSecurityConfig {
  ...
  public SecurityFilterChain filterChain (HttpSecurity http) throws Exception {
    ...
    http
        .cors(Customizer.withDefaults()) // withDefaults(): Bean으로 등록된 corsConfigurationSource()라는 메소드를 찾아서 실행
        //.cors(cors->cors.configurationSource(corsConfigurationSource())) // 메소드명을 변경하고 싶을 경우
        .csrf(CsrfConfigurer::disable) // 외부 post, put 요청을 허용
        .logout(auth -> auth
        ...
        .authorizeHttpRequests(authorize -> authorize
            ...
            .anyRequest().authenticated()); // 위에서 걸러지지 않은 api는 모두 인증을 필요로 함

    // permitAll(): 인증 없이 접근 가능
    // hasRole(): 특정 role이 있어야 접근 가능 (ex. admin, hasRole("admin"))
    // hasAnyRole(): 파라미터로 넘긴 role 중 하나를 만족 (ex. hasAnyRole("admin", "teacher"))
  ...
  public CorsConfigurationSource corsConfigurationSource () {
    ...
    config.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "PATCH", "DELETE", "HEAD"));
